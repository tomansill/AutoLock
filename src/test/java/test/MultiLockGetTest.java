...
interface MultiLockGetTest {
	...
	@DisplayName("multi-lock-get: with Throwable thrown in supplier AND Throwable thrown in unlock() on 4th lock")
	@TestFactory
	default Iterable<DynamicTest> testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod4th() {
		return testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(3);
	}

	@DisplayName("multi-lock-get: with Throwable thrown in supplier AND Throwable thrown in unlock() on 3rd lock")
	@TestFactory
	default Iterable<DynamicTest> testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod3rd() {
		return testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(2);
	}

	@DisplayName("multi-lock-get: with Throwable thrown in supplier AND Throwable thrown in unlock() on 2nd lock")
	@TestFactory
	default Iterable<DynamicTest> testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod2nd() {
		return testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(1);
	}

	@DisplayName("multi-lock-get: with Throwable thrown in supplier AND Throwable thrown in unlock() on 1st lock")
	@TestFactory
	default Iterable<DynamicTest> testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod1st() {
		return testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(3);
	}

	default Iterable<DynamicTest> testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(int position) {
		...
		return throwableMap.entrySet().stream().map(entry -> {
			...
			return DynamicTest.dynamicTest(entry.getKey(), () -> this.testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(mainThrowable, entry.getValue(), position));
		}).collect(Collectors.toList());
	}

	default void testMultiLockGet_ThrowableInSupplierAndThrowableInUnlockMethod(
